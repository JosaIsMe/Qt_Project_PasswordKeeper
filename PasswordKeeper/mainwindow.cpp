...
{
    createReadOnlyTextDialog(this,"Help:Generator","密码生成器帮助","密码生成器可以帮助您生成一个包含指定字符的随机密码，只需轻轻点击Generate按钮。下方的四个复选框允许用户根据自己的需求改变密码可以选择的字符，而右侧的size框可以调节密码的长度。");
}

void MainWindow::on_actionPasswords_triggered()
{
    createReadOnlyTextDialog(this,"Help:Passwords","密码管理界面帮助","密码管理界面可以帮助您安全地管理您的密码。左侧的密码搜索框允许用户搜索匹配的站点，只需要向搜索框输入站点的名称（可以是部分名称）并点击Search按钮即可实现。下方的栏目中会列出符合匹配的站点名称，用户名和密码。不输入任何文字直接按Search会显示所有密码。\n显示密码后，密码单元格的背景颜色会提示您密码的强度，红色代表弱，绿色代表强，而黄色代表适中。建议您选用更强的代码以保证安全性。\n在右侧输入站点，用户名和密码，按下Add，Update和Remove三个键即可添加，更改与删除存储的密码。");
}

void MainWindow::on_actionTexts_triggered()
{
    createReadOnlyTextDialog(this,"Help:Texts","文本加密界面帮助","文本加密界面会使用独属于您的主密钥加密文本，其他人的密钥无法解密您的密钥加密后的文本。\n将待加密的文本输入左侧的文本框中，点击Encryption即可在右侧获得加密后的文本。同样，将加密后的文本输入右侧的文本框中，点击Decryption可以在左侧获得解密的文本。");
}

void MainWindow::on_actionFiles_triggered()
{
    createReadOnlyTextDialog(this,"Help:Files","文件加密页面帮助","文件加密页面会使用独属于您的主密钥加密用于加密文件的密钥。您需要向Input Path输入待加密文件的路径，向Output Path输入目标文件的路径（不存在这样的目标文件时会自动创建），以及向Identifier输入您为这个文件提供的内部名称。提供后内部名称和生成的密钥将隐式存于密码管理系统中。点击Encryption会将待加密文件加密，生成加密后的文件。同样，将待解密文件路径输入Input Path，解密到的文件路径输入Output Path, 向Identifier输入您想要使用的密钥条目，点击Decryption会将文件解密。");
}
